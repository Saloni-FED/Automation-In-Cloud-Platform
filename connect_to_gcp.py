...
def deploy_function_and_create_trigger():
    ...
    try:
        ...
        # Check if the object exists (function-source.zip) before uploading
        blob = bucket.blob(f"{source}/function-source.zip")
        if not blob.exists():
            # If the file doesn't exist, upload the new function-source.zip file
            blob.upload_from_filename(zip_file_path)
            app.logger.info(f"File {zip_file_path} uploaded to bucket {bucket_name_create} under path {source}/function-source.zip")
        else:
            # If the file exists, do not upload again
            app.logger.info(f"File {source}/function-source.zip already exists in bucket {bucket_name_create}")
    ...
    try:
        ...
        function.build_config.source.storage_source.object_ = f"{source}/function-source.zip"
